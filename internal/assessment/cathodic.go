...
func NewCathodicAssessment(cable, window string, spans []string, signals map[string]float64) CathodicAssessment {
	...
	return CathodicAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a CathodicAssessment) NormalizeSpans() []string {
...
func (a CathodicAssessment) Result() CathodicResult {
	...
	return CathodicResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a CathodicAssessment) Merge(other CathodicAssessment) CathodicAssessment {
	out := NewCathodicAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
