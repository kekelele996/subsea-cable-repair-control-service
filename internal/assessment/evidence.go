...
func NewEvidenceAssessment(cable, window string, spans []string, signals map[string]float64) EvidenceAssessment {
	...
	return EvidenceAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a EvidenceAssessment) NormalizeSpans() []string {
...
func (a EvidenceAssessment) Result() EvidenceResult {
	...
	return EvidenceResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a EvidenceAssessment) Merge(other EvidenceAssessment) EvidenceAssessment {
	out := NewEvidenceAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
