...
func NewPermitAssessment(cable, window string, spans []string, signals map[string]float64) PermitAssessment {
	...
	return PermitAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a PermitAssessment) NormalizeSpans() []string {
...
func (a PermitAssessment) Result() PermitResult {
	...
	return PermitResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a PermitAssessment) Merge(other PermitAssessment) PermitAssessment {
	out := NewPermitAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
