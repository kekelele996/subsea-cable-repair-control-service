...
func NewBarrierAssessment(cable, window string, spans []string, signals map[string]float64) BarrierAssessment {
	...
	return BarrierAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a BarrierAssessment) NormalizeSpans() []string {
...
func (a BarrierAssessment) Result() BarrierResult {
	...
	return BarrierResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a BarrierAssessment) Merge(other BarrierAssessment) BarrierAssessment {
	out := NewBarrierAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
