...
func NewFatigueAssessment(cable, window string, spans []string, signals map[string]float64) FatigueAssessment {
	...
	return FatigueAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a FatigueAssessment) NormalizeSpans() []string {
...
func (a FatigueAssessment) Result() FatigueResult {
	...
	return FatigueResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a FatigueAssessment) Merge(other FatigueAssessment) FatigueAssessment {
	out := NewFatigueAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
