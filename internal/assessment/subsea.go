...
func NewSubseaAssessment(cable, window string, spans []string, signals map[string]float64) SubseaAssessment {
	...
	return SubseaAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a SubseaAssessment) NormalizeSpans() []string {
...
func (a SubseaAssessment) Result() SubseaResult {
	...
	return SubseaResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a SubseaAssessment) Merge(other SubseaAssessment) SubseaAssessment {
	out := NewSubseaAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
