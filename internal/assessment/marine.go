...
func NewMarineAssessment(cable, window string, spans []string, signals map[string]float64) MarineAssessment {
	...
	return MarineAssessment{CableID: strings.TrimSpace(cable), Window: strings.TrimSpace(window), Spans: copied, Signals: signalCopy}
}

func (a MarineAssessment) NormalizeSpans() []string {
...
func (a MarineAssessment) Result() MarineResult {
	...
	return MarineResult{Score: score, Band: band, Review: score >= 18, Reasons: reasons, Normalized: a.NormalizeSpans()}
}

func (a MarineAssessment) Merge(other MarineAssessment) MarineAssessment {
	out := NewMarineAssessment(a.CableID, a.Window, a.Spans, a.Signals)
	if out.CableID == "" {
